...
const handleErrors = (err, res) => {
  ...
  if (errorStatus == null && res && res.status === 500) {
    Utils.Alert({
      ...
      message: Utils.Translate("Gerekli moduller yüklenemedi. Lütfen tekrar deneyiniz. :errorText", {
        errorText: errorText
      }),
      level: "error",
      autoDismiss: 5
  ...
  if (err) {
    Utils.Alert({
      position: "tc",
      title: Utils.Translate("Hata oluştu"),
      message: Utils.Translate("İşlem sırasında hata oluştu. Lütfen tekrar deneyiniz.:errorText", {
        errorText: errorText
      }),
      level: "error",
      autoDismiss: 5
...
const Request = {
  timeout: 30000,
  deadline: 60000,
  del: url => superagent.del(`${API_ROOT_URL}${url}`).use(tokenPlugin).on("error", handleErrors).end(handleErrors).then(responseBody),
  get: (url, params = {}) => {
    ...
    return superagent.get(`${fullUrl}`).query(params).use(tokenPlugin).end(handleErrors).then(responseBody);
  },
  put: (url, body) => superagent.put(`${API_ROOT_URL}${url}`, body).use(tokenPlugin).on("error", handleErrors).end(handleErrors).then(responseBody),
  ...
  postJson: (url, body) => {
    ...
    return superagent.post(fullUrl, body).use(tokenPlugin).on("error", handleErrors).end(handleErrors).then(responseBody);
  },
  postForm: (url, body) => {
    var fullUrl = `${API_ROOT_URL}${url}`;
    ...
    return superagent.post(fullUrl, body).type("form").use(tokenPlugin).on("error", handleErrors).end(handleErrors).then(responseBody);
  },
