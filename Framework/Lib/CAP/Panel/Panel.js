...
export default class Panel extends React.Component {
  ...
  render() {
    ...
    //items var ise
    if (children == null && this.props.items.length > 0)
      children = this.props.items.map(e => {
        return e;
      });

    //config içinde items var ise
    if (children == null && this.props.config.items.length > 0)
      children = this.props.config.items.map((E, i) => {
        if ((typeof  E.$$typeof) == "symbol")
          return <React.Fragment key={this.key + "-child-item-" + i}>{E}</React.Fragment>;
