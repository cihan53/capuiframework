...
export default class CapController extends React.Component {
    ...
    init() {
        ...
        CAP.Logger.debug({controller: c, action: a, cp: this._components})
    ...
    renderView = (view, data = {}, loadmask = false, controller = this.controllerName) => {
        ...
        if (Utils.startsWith(view, "/") || Utils.startsWith(view, "..")) {
        ...
        Logger.debug(`@ThemeViewsPath/${_view}`);
        if (!loadmask) {
            let View = Loadable({
                loader: () => import(
                    /* webpackMode: "lazy" */
                    `@ThemeViewsPath/${_view}`),
    ...
    addError = (error) => {
        CapController._errors.push(error);
        console.error(error);
        return this;
    }
    ...
    getUrlParams = (paramString = ":id?") => {
        ...
        const match = matchPath(this.props.location.pathname, {
            path: Utils.endsWith(this.props.match.path, paramString) ? this.props.match.path : this.props.match.path + paramString,
            exact: true,
            strict: false
        }) || {};
     ...
     * @returns {string}
     */
    static _createUrl = (actionName, params = {}, hash = true) => {
    ...
    createUrl(actionName, params = {}, hash = true) {
        return CapController._createUrl(actionName, params, true)
    }
    ...
    static _toUrl(actionName, params = {}) {
        return window.location = this._createUrl(actionName, params);
    }
